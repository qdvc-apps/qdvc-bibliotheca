...
- **Catalogue tab** — three-pane master/detail:
  - Sidebar (with icons): All articles / By type / My works / Starred authors,
    ...
    right-click a work to edit it (or double-click it). Right-click behaviour
    replaces the old button bar.
  - Master: sortable, filterable table of records. Articles that have a PDF
    on file show a PDF icon in the first column.
  - Detail: APA 7 reference (with **Copy rich** and **Copy plain text**), an
    editable Markdown notes box (auto-saves on record change / close), and
    **Set PDF / Set EPUB / Clear** buttons for attaching full-text files.
- **Authors tab** — a unique list of authors derived from your BibTeX. Each
  ...
  sidebar. Double-click an author (or use *Show works in Catalogue*) to filter
  the catalogue to that author's works.
...
- **Full-text library** — set a base folder in Preferences ("Full-text library
  ...
  *relative* to that folder (falling back to an absolute path, with a notice,
  if the file is outside it). This keeps a workspace portable.
- **Menubar**
  - *File*: Open / Close / Recent workspaces, Import BibTeX, Quit
  - *Edit*: Preferences (editor font, file-manager command, full-text library
    path, startup + autosave)
